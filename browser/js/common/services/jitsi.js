app.service('jitsi', function() {
    var options = {
        hosts: {
            domain: 'humantics.build',
            muc: 'conference.humantics.build', // FIXME: use XEP-0030
            bridge: 'jitsi-videobridge.humantics.build', // FIXME: use XEP-0030
        },
        bosh: 'https://humantics.build/http-bind', // FIXME: use xep-0156 for that
    ...
    function onLocalTracks(tracks) {
        ...
        for (var i = 0; i < localTracks.length; i++) {
            ...
            if (localTracks[i].getType() == "video") {
                $("jitsi").append("<video autoplay='1' id='localVideo" + i + "' />");
                localTracks[i].attach($("#localVideo" + i)[0]);
            } else {
                $("jitsi").append("<audio autoplay='1' muted='true' id='localAudio" + i + "' />");
    ...
    function onRemoteTrack(track) {
        ...
        if (track.getType() == "video") {
            $("jitsi").append("<video autoplay='1' id='" + participant + "video" + idx + "' />");
        } else {
            $("jitsi").append("<audio autoplay='1' id='" + participant + "audio" + idx + "' />");
        }
        track.attach($("#" + id)[0]);
    }
